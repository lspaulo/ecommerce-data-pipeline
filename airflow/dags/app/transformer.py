...
def transformer_process_from_bronze_to_silver():
    ...
    print(df_droped_delivered.info())

    print("Saving Data Info Silver Layer")
    pq.write_to_dataset(
        pyarrow.Table.from_pandas(df_droped_delivered),
        'bucket-testing-dnc-btfg/ecommerce/silver/fact_table',
        filesystem=s3fs.S3FileSystem()
    )

def orders_to_logistic():
    print("Initializing data transformation to logistic")
    ...
    for obj in response.get('Contents', []):
        ...
        if ".parquet" in file_key:
            ...
            # Read the Parquet file using pyarrow and convert it to a Pandas Dataset
            df = pq.read_table(parquet_file).to_pandas()

            # Append the Daraframe to the list
            dfs.append(df)
    ...
    print("Loading Custmer's Dimension")
    ...
    df_merged["state_city"] = df_merged["customer_state"].str.cat(df_merged["customer_city"], sep="-")
    ...
    print("Saving Data Info Golden Layer")

    pq.write_to_dataset(
        pyarrow.Table.from_pandas(df_merged_grouped),
        'bucket-testing-dnc-btfg/ecommerce/gold/logistic',
        filesystem=s3fs.S3FileSystem()
    )

def orders_to_finance():
    print("Initializing data transformation to finance")
    ...
    print("Loading Sale's Data")
    file_key = "ecommerce/silver/order_details/arquivo.parquet"
    response = s3_client.get_object(
        Bucket=bucket_name,
        Key=file_key
    )
    ...
    pq.write_to_dataset(
        pyarrow.Table.from_pandas(df_merged_grouped),
        'bucket-testing-dnc-btfg/ecommerce/gold/finance',
        filesystem=s3fs.S3FileSystem()
    )
